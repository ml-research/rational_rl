...
def test(args, model, device, test_loader, epoch, tests_losses, tests_acc):
    model.eval()
    test_loss = 0
    correct = 0
    with torch.no_grad():
...
def main():
    ...
    for activation_function_key in actfvs.keys():
        ...
        test(args, model, device, test_loader, 0, tests_losses, tests_acc)
        for epoch in range(1, args.epochs + 1):
            train(args, model, device, train_loader, optimizer, optimizer_activation, None, epoch, losses)
            test(args, model, device, test_loader, epoch, tests_losses, tests_acc)
        ...
        with open(f"scores_sl/{args.arch}_scores_{args.dataset}/scores_{args.arch}_{activation_function_key}_{args.seed}.pkl", "wb") as scoref:
            pickle.dump((tests_losses, tests_acc), scoref)
